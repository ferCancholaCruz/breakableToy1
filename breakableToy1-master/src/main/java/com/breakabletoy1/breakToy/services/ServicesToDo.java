...
public class ServicesToDo {
    ...
    private Comparator<ToDo> getSort(String order) {
        if (order == null || order.isEmpty()) {
            return Comparator.comparingInt(ToDo::getID);
        }
        ...
        return switch (order) {
            ...
            case "BothDesc" -> Collections.reverseOrder(new SortBoth()); // ✅ fix aplicado aquí
            case "PriorityAscDueDesc" -> priorityThenDueDate(true, false);
            case "PriorityDescDueAsc" -> priorityThenDueDate(false, true);
            default -> Comparator.comparingInt(ToDo::getID);
        };
    }
    ...
    private ToDo getOrThrow(int id) {
