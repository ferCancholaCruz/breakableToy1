...
import { fetchTasks, markTaskDone, markTaskUndone, deleteTask, createTask, updateTask } from "../services/TaskService";
...
const TodoList = () => {
  ...
  useEffect(() => {
    if (tasks.length > 0) calculateAverages(tasks);
  }, [tasks]);
  ...
  const loadTasks = async (order?: string, filters: string = "") => {
    ...
    console.log("Tareas transformadas:", transformed);
